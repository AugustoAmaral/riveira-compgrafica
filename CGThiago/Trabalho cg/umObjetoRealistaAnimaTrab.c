...
float gConsRest = 1.0;	// [0.0, 1.0]constante de restitucao da face de colisao
	
float gKd = 0.8;	// mola: constante de damp
float gKs = 0.6;    // mola constante de spring
float gMassa = 0.5; // masa da particula;
f4d	gGrav = {-1.0, -9.8, -1.0};	// acelracao de gravidade (0, -9.8, 0) dado em inicial
float gDt = 0.01;	// passo de tempo
...
float cor1[] ={0.0, 1.0, 0.0};
float cor2[] = {0.0, 1.0,1.0};
//============================================================
...
void DesenhaObjeto(umObjeto *obj, float *cor)
{
	int i, j, h;
	f4d vn;
...
static void passoEDOresolve(umObjeto *obj) {
	...
	for(i=0; i<dinParts->n; i++) { 
		...
		// tmpVarPos = temVarPos + pos;
			...
			tmpVarPos[0] += obj->Vertices->vPos[i][0]; 
			tmpVarPos[1] += obj->Vertices->vPos[i][1]; 
			tmpVarPos[2] += obj->Vertices->vPos[i][2]; 
		...
		//
		 //tmpVarVel = tmpVarVel + vel;
			...
			tmpVarVel[0] += dinParts->mvf[i].vel[0];
			tmpVarVel[1] += dinParts->mvf[i].vel[1];
			tmpVarVel[2] += dinParts->mvf[i].vel[2];
...
// Default material settings are used. You may define your own material properties.
	...
	if (objeto) {
		if(gAnima) {
			animaObjeto(objeto);
			DesenhaObjeto(objFixo, cor2);
		}
		DesenhaObjeto(objeto, cor1);
	}
} 
